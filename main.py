...
# A scale word tells us a table's numbers are abbreviated, e.g. "(Dollars in Millions)".
SCALE = {
    "thousand": 1_000,
    "million": 1_000_000,
...
# Matches the scale word inside a header like "(Dollars in Millions)" or "($ in Thousands)".
_SCALE_PHRASE = re.compile(r"\bin\s+(thousand|million|billion|trillion)s?\b", re.IGNORECASE)

# A number such as 12, 3.15, or 1,234,567.8 (optional thousands commas and decimal part).
...
def detect_scale(text):
    """Return the multiplier a line declares, e.g. "(Dollars in Millions)" -> 1_000_000.
...
def find_largest(pages):
    """Return the largest raw value and the largest scale-adjusted value in the document.
...
def snippet(page_text, line_index, count=6):
    """Return the last few non-empty lines up to a hit, so a result can be eyeballed."""
    earlier = [line.strip() for line in page_text.splitlines()[: line_index + 1] if line.strip()]
    return " | ".join(earlier[-count:])
...
if __name__ == "__main__":
    ...
    print(f"    page {raw_page}: {snippet(pages[raw_page - 1], raw_line)}")
    print()
    print(f"Largest adjusted value: {adj_value:>20,.0f}")
    print(f"    page {adj_page}: {snippet(pages[adj_page - 1], adj_line)}")
